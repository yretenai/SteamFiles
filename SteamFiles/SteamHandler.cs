...
public class SteamHandler {
    ...
    private Dictionary<string, List<Regex>> Rules { get; }

    private SteamUser.LogOnDetails LogonDetails { get; } = new();
    ...
    private SteamClient Steam { get; }
    private SteamUser User { get; }
    private SteamApps Apps { get; }
    private SteamContent Content { get; }
    private CallbackManager Callbacks { get; }
    private Thread? CallbackThread { get; set; }
    ...
    private void LicenseListCallback(SteamApps.LicenseListCallback licenses) {
        var call = LicenseListCallbackAsync(licenses);
        call.Wait();
    }

    private async Task LicenseListCallbackAsync(SteamApps.LicenseListCallback licenses) {
        ...
        var appPics = await Apps.PICSGetProductInfo(appIds.Select(x => new SteamApps.PICSRequest { AccessToken = appTokens.TryGetValue(x, out var token) ? token : 0, ID = x }), ArraySegment<SteamApps.PICSRequest>.Empty);
    ...
    private async static Task SaveTags(Dictionary<string, Dictionary<uint, string>> tags) {
        ...
        foreach (var (category, appIds) in tags.OrderBy(x => x.Key)) {
            ...
            foreach (var (appId, appName) in appIds.OrderBy(x => x.Key)) {
                await writer.WriteLineAsync($"{appId} = \"{appName.Replace('"', '\'')}\"");
            }
    ...
    private async Task ProcessApp(uint appId, KeyValue app, HashSet<uint> depotIds, Dictionary<string, Dictionary<uint, string>> tags, Server? cdn, Client cdnClient) {
        ...
        foreach (var (depotId, manifest) in manifests) {
            ...
            if (!File.Exists(manifestPath)) {
                ...
                files = (manifestData.Files ?? []).Select(x => x.FileName.Replace("\\", "/")).ToArray();
